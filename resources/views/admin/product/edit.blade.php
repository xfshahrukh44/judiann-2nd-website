                            ...
                            <div class="card card-primary">
                                <div class="card-header">
                                    <ul class="nav nav-tabs">
                                        ...
                                        <li class="nav-item">
                                            <a class="nav-link" href="#productOptions" role="tab" data-toggle="tab">Product
                                                Options</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" href="#attributes"
                                               role="tab" data-toggle="tab">Product Addons</a>
                                        </li>
                                        {{-- <li class="nav-item">--}}
...
{{--                                            </div>--}}
                                        ...
                                        <div class="tab-pane" role="tabpanel" id="productOptions">
...
{{--                                                    <th>Qty</th>--}}
                                                ...
                                                @foreach($product->products_options as $key => $products_options)
                                                    <tr id="row_option_{{$key}}">
                                                        <td>
                                                            <select id="option_id" class="form-control"
                                                                    name="option_id[]"
                                                                    onchange="getOptionValues({{ $key + 1 }},this.value)">
                                                                <option value="">Select Option</option>
                                                                @foreach($options as $option)
                                                                    <option
                                                                        value="{{$option->id}}" {{ $products_options->option_id == $option->id ? 'selected' : '' }}>{{$option->option_name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <select id="option_value_id_{{$key + 1}}"
                                                                    class="form-control"
                                                                    name="option_value_id[]">
                                                                @foreach($option_values as $option_val)
                                                                    @if($products_options->option_id == $option_val->option_id)
                                                                        <option
                                                                            value="{{ $option_val->id }}" {{ $products_options->option_val_id == $option_val->id ? 'selected' : '' }}>{{ $option_val->option_value }}</option>
                                                                    @endif
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="number" class="form-control"
                                                                   value="{{ $products_options->price }}"
                                                                   name="option_value_price[]" step="0.01">
                                                        </td>
                                                        <td hidden><input type="number" class="form-control"
                                                                   value="{{ $products_options->qty }}"
                                                                   name="option_value_qty[]"></td>
                                                        <td><input type="button" class="btn btn-danger btn-md" value="-"
                                                                   onclick="removeOptionRow({{ $key }})"></td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
                                            </table>
                                        ...
                                        <div class="tab-pane" role="tabpanel" class="tab-pane fade in active" id="attributes">
                                            <div class="col-md-12 text-right">
                                                <input type="button" class="btn btn-primary btn-sm" value="Add Product Addon" onclick="addMoreAttributes()" style="margin-top: 10px;margin-bottom: 10px;">
                                            </div>
                                            <table class="table table-bordered">
                                                <thead>
                                                <tr>
                                                ...
                                                @foreach($product->products_attributes as $key => $products_attribute)
                                                    <tr id="row_attr_{{ $key + 1 }}">
                                                        <td>
                                                                ...
                                                                @foreach($attributeGroups as $attributeGroup)
                                                                    <optgroup label="{{$attributeGroup->attribute_group}}">
                                                                        @foreach($attributeGroup->attributes as $attributes)
                                                                            <option value="{{$attributes->id}}" {{ $products_attribute->attribute_id == $attributes->id ? 'selected' : '' }}>{{$attributes->attribute_name}}</option>
                                                                        @endforeach
                                                                    </optgroup>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td><input type="number" step="0.01" class="form-control" value="{{$products_attribute->value}}" name="attribute_value[]" required></td>
                                                        <td>
                                                            <input type="button" class="btn btn-danger btn-md" value="-" onclick="removeAttrRow({{ $key + 1 }})" id="add_more_attr_btn">
                                                        </td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function getOptionValues(counter, val) {
            ...
            if (option_id !== '') {
                $.ajax({
                    ...
                    success: function (data) {
                        console.log('#option_value_id_' + counter);
        ...
        var counterO = {{ $product->products_options->count() ?? 0 }};

        function addMoreOptions() {
            counterO++;
            $('#add_more_option').append(`<tr id="row_option_${counterO}"><td>
                                            <select id="option_id_${counterO}" class="form-control" name="option_id[]" onchange="getOptionValues(${counterO},this.value)">
                                                <option value="">Select Option</option>
                                                @foreach($options as $option)
            ...
            <td>
                <select id="option_value_id_${counterO}" class="form-control" name="option_value_id[]" ></select>
                                            </td>
                                            <td><input type="number" class="form-control" value="" step="0.01" name="option_value_price[]"></td>
                                            <td hidden><input type="number" class="form-control" value="0" name="option_value_qty[]"></td>
        ...
        // Product Attributes
        var counter1 = {{ $product->products_attributes->count() ?? 0 }}
        function addMoreAttributes(){
            counter1++;
            $('#add_more_attr').append(`<tr id="row_attr_${counter1}">
                                        <td>
                                            <select id="dino-select" class="form-control" name="attribute[]">
                                        ...
                                        <td><input type="number" step="0.01" class="form-control" value="" name="attribute_value[]"></td>
